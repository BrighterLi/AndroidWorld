package com.tencent.tecentim.order.tabLayout;
 ...
 *
 * https://github.com/JasonGaoH/XTabLayout
 */
...
public class XTabLayout extends HorizontalScrollView {
    ...
    public void setTabMinimumWidth(int minWidth) {
        mRequestedTabMinWidth = minWidth;
        requestLayout();
    }
    ...
    //尝试添加滚动到指定position的方法,有待完善
    public void moveToPosition(final int newPosition){
        post(new Runnable() {
            @RequiresApi(api = Build.VERSION_CODES.KITKAT)
            @Override
            public void run() {
                int scrollX = calculateScrollXForTab(newPosition, 0);
                View childAt = Objects.requireNonNull(getTabAt(newPosition)).getCustomView();
                int selectedWidth = childAt != null ? childAt.getLayoutParams().width : 0;
                smoothScrollTo(scrollX + selectedWidth, 0);
            }
        });
